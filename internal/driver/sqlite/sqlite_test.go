...
func TestSQLiteDriver_Execute_Select(t *testing.T) {
	driver := New()
	cfg := connection.DatabaseConnection{
		Database: ":memory:",
	}
...
func TestSQLiteDriver_Execute_Insert(t *testing.T) {
	driver := New()
	cfg := connection.DatabaseConnection{
		Database: ":memory:",
	}
	...
	_, err = driver.Execute(context.Background(), "CREATE TABLE test (id INTEGER PRIMARY KEY, name TEXT)")
	if err != nil {
		t.Fatalf("Create table failed: %v", err)
	}

	result, err := driver.Execute(context.Background(), "INSERT INTO test (name) VALUES ('test')")
	if err != nil {
...
func TestSQLiteDriver_GetSchema(t *testing.T) {
	driver := New()
	cfg := connection.DatabaseConnection{
		Database: ":memory:",
	}
	...
	_, err = driver.Execute(ctx, "CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT)")
	if err != nil {
		t.Fatalf("Create table failed: %v", err)
	}
...
func TestSQLiteDriver_GetColumns(t *testing.T) {
	...
	_, err = driver.Execute(context.Background(), "CREATE TABLE users (id INTEGER PRIMARY KEY, name TEXT NOT NULL, age INTEGER DEFAULT 18)")
	if err != nil {
		t.Fatalf("Create table failed: %v", err)
	}
