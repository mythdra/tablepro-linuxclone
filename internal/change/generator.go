...
func (g *SQLStatementGenerator) GenerateUpdate(change *CellChange, schemaName, tableName string) (string, []any, error) {
	...
	args := make([]any, 0, 1+len(change.PrimaryKeys))
	paramIdx := 1

	b.WriteString("UPDATE ")
	b.WriteString(g.qualifiedTable(schemaName, tableName))
	b.WriteString(" SET ")
	b.WriteString(g.Dialect.QuoteIdentifier(change.ColumnName))
	b.WriteString(" = ")
	...
	b.WriteString(" WHERE ")
	pkKeys := sortedKeys(change.PrimaryKeys)
	for i, pk := range pkKeys {
		...
		b.WriteString(" = ")
		b.WriteString(g.Dialect.ParamMarker(paramIdx))
		args = append(args, change.PrimaryKeys[pk])
		paramIdx++
	}
...
func (g *SQLStatementGenerator) GenerateDelete(row *DeletedRow, schemaName, tableName string) (string, []any, error) {
	...
	var b strings.Builder
	args := make([]any, 0, len(row.PrimaryKeys))
	paramIdx := 1
	...
	pkKeys := sortedKeys(row.PrimaryKeys)
	for i, pk := range pkKeys {
		if i > 0 {
			b.WriteString(" AND ")
		}
		...
		b.WriteString(" = ")
		b.WriteString(g.Dialect.ParamMarker(paramIdx))
		args = append(args, row.PrimaryKeys[pk])
		paramIdx++
	}
